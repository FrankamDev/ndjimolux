    ...
    <div className="mt-8 pt-6 border-t border-gray-800">
      ...
      {auth.user ? (
        <div className="flex gap-3 mb-8">
          <div className="w-8 h-8 rounded-full bg-emerald-700 flex-shrink-0 flex items-center justify-center text-xs font-bold text-white">
            {auth.user.profile_photo_url ? (
              <img src={auth.user.profile_photo_url} className="w-full h-full rounded-full object-cover" />
            ) : (
        ...
        <div className="bg-gray-900 rounded-lg p-4 mb-8 text-center">
          ...
          <a href="/login" className="text-emerald-400 hover:underline text-sm font-bold mt-2 inline-block">Se connecter</a>
        </div>
      )}
